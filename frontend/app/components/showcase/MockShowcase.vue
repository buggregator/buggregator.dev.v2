  ...
  <section id="showcase" class="py-20 lg:py-28 bg-white">
    <div class="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div class="text-center mb-12">
        <h2 class="text-section font-bold text-on-light-primary mb-3">
          {{ t('showcase.title') }}
        </h2>
        <p class="text-lg text-on-light-muted">
          {{ t('showcase.subtitle') }}
        </p>
      </div>

      <!-- Showcase container -->
      <div class="mx-auto bg-white border border-[#e5e7eb] rounded-2xl overflow-hidden shadow-showcase">
        <!-- Tab bar -->
        <div
          class="flex overflow-x-auto bg-[#f3f4f6] px-3 py-2.5 border-b border-[#e5e7eb] gap-1"
          role="tablist"
        >
          <button
            v-for="tab in tabs"
        ...
        <div class="px-5 py-3 bg-[#f9fafb] border-b border-[#e5e7eb]">
          <p class="text-sm text-on-light-muted">{{ activeDescription }}</p>
        </div>
